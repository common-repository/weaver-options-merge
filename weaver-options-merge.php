...
Text Domain: weaver-options-merge
Domain Path: /lang
Version: 0.4
License: GPL
...
load_plugin_textdomain('weaver-options-merge', FALSE, dirname(plugin_basename(__FILE__)) . '/lang');
...
function wom_merge_add_admin_menu() {
	...
	$my_page = add_management_page(__( 'Weaver Options Merge', 'weaver-options-merge' ), __( 'Weaver Options Merge', 'weaver-options-merge' ),
...
function wom_merge_load_scripts() {
	...
	$scriptmsgs = array(
		'nothing' => __( 'Nothing to merge', 'weaver-options-merge' ),
		'needtwo' => __('Need two option sets to merge', 'weaver-options-merge' ),
		'allselbutton' => __('All', 'weaver-options-merge' ),
		'colorselbutton' => __('Colors', 'weaver-options-merge' ),
		'clrcssselbutton' => 'CSS+',
		'insselbutton' => __('insert', 'weaver-options-merge' ),
		'fontselbutton' => __('font', 'weaver-options-merge' ),
		'siteselbutton' => __('Site', 'weaver-options-merge' ),
		);
...
function wom_merge_create_option_list( $theme_abbrev, $radio ) {
	...
	if ( current_user_can( 'manage_options' ) ) {
		...
		if ( $theme_options ) {
			$content[0] .= '<h4>'.__( 'Database option sets:', 'weaver-options-merge' ) .'</h4><div>';
			$content[1] .= '<h4>'.__( 'Database option sets:', 'weaver-options-merge' ) .'</h4><div>';
			foreach ( $theme_options as $row ) {
				$name = esc_html( ucwords( str_replace( '_', ' ', $row->option_name ) ) );
				if ( $theme_settings_name == $row->option_name )
					$name = __( 'Current', 'weaver-options-merge' ).' '.$name;
		...
		if ( $theme_options ) {
			$content[0] .= '<h4>'.__( 'Theme Switcher option sets:', 'weaver-options-merge' ) .'</h4><div>';
			$content[1] .= '<h4>'.__( 'Theme Switcher option sets:', 'weaver-options-merge' ) .'</h4><div>';
...
function wom_merge_create_file_list( $theme_name, $radio ) {
	...
	if ( current_user_can( 'manage_options' ) ) {
		wom_merge_list_file_inputs( wp_normalize_path( $upload_dir['basedir'] ).'/'.$theme_abbrev.'-subthemes',
				__( 'Saved option sets:', 'weaver-options-merge' ), $radio,	$content );
	}
	wom_merge_list_file_inputs( wp_normalize_path( $theme_info->get_stylesheet_directory() ),
			__( 'Theme built-in option sets:', 'weaver-options-merge' ), $radio, $content );
	$content[0] .= '<h4>'.__( 'Upload:', 'weaver-options-merge' ) .'</h4><div>';
	$content[1] .= '<h4>'.__( 'Upload:', 'weaver-options-merge' ) .'</h4><div>';
	...
	$content[0] .= '<label for="'.$prefix.'_womfile0">'.__( 'Upload a settings file', 'weaver-options-merge' ).' <input type="file" name="'.$prefix.'_settings_file0" value="" /></label></p>';
	$content[0] .= '</div>';
	...
	$content[1] .= '<label for="'.$prefix.'_womfile1">'.__( 'Upload a settings file', 'weaver-options-merge' ).' <input type="file" name="'.$prefix.'_settings_file1" value="" /></label></p>';
	$content[1] .= '</div>';
	$temp = array();	$temp[0] = $temp[1] = '';
	wom_merge_list_file_inputs( wp_normalize_path( plugin_dir_path( __FILE__ ) ).'optionsets',
			__( 'Plugin provided option sets:', 'weaver-options-merge' ), $radio,	$temp );
	$content[1] .= $temp[1];
...
function wom_merge_create_list_form( $theme_name ) {
	$theme_abbrev = wom_merge_abbrev( $theme_name );
	$prefix = 'wom_'.$theme_abbrev;
	$content = '<p>'. __( 'Choose two option sets to show. The left set will show as selected by default, for the merge.', 'weaver-options-merge' ) .'</p>';
  ...
  $bothfiles = wom_merge_create_file_list( $theme_name, $prefix.'_optset' );
	...
	$content .= '<p><input type="submit" class="button-primary womshowmerge" name="'.$prefix.'_showmerge" value="'. __( 'Show side by side', 'weaver-options-merge' ).'" data-womradio="'.$prefix.'_optset" /></p>';
	$content .= '</form>';
...
function wom_merge_create_merge_table( $basetomerge, $othertomerge, $headingrow, $backupkeys ) {
	...
	foreach( $basetomerge as $k=>$v ) {
		...
		if ( $merge )
			$out .= '<td><div class="womkey"><label>'. esc_html( $k ) .'<br /><span>'. __( 'Merge', 'weaver-options-merge' ).'</span>' . ($merge? wom_merge_radio( $k, 3 ) : '')."</label></div></td>";
		else
			$out .= '<td><div class="womkey">'. esc_html( $k ) .'</div></td>';
...
function wom_merge_create_merge_form( $theme_name, $baseset, $otherset ) {
	...
	$out .= '<p class="wombackup">'.__( 'Site-specific options (&diams;) have a darker background color.', 'weaver-options-merge' ).'</p>';
	if ( count( $basetomerge ) > 0 && count( $othertomerge ) > 0) {
		$out .= wp_nonce_field( 'wommergefile', $theme_abbrev.'mergeIDfile', true, false );
		$out .= '<p><input type="submit" class="button-primary" id="wom_merge_files" name="wom_merge_files" value="'. __( 'Merge Files', 'weaver-options-merge' ).'"/></p>';
		$instruct = '<p>'.__( 'Choose the options to write to the merged file. The buttons below select subsets.', 'weaver-options-merge' )."</p>\n";
	}
	else $instruct = '';
	$out .= '</form>';
...
function wom_merge_load_option_set( $theme_name, $postedname, $postedfile ) {
	...
	if ( isset( $_POST[$postedname] ) ) {
		if ( strpos( $_POST[$postedname], '+womfile+' ) === 0 )	{
			...
			else $msg = __( 'Uploaded file is missing.', 'weaver-options-merge' );
		}
		else {
			...
			if ( current_user_can( 'manage_options' ) ) {
				$theme_settings_name = $theme_abbrev.( $theme_abbrev=='weaver' ? '_main' : '' ).'_settings';
				$switcher = $theme_abbrev.'_switch_themes';
				$optname = explode( '+wom+', $_POST[$postedname], 2 );
				$option = sanitize_key( $optname[0] );
				$suboption = isset( $optname[1] ) ? esc_html( $optname[1] ) : '';
				...
				if ( in_array( $option, $valid_list ) ) {
					if ( $suboption ) {
						...
						if ( array_key_exists( $suboption, $data ) ) {
							...
							$msg = sprintf( __( 'Theme switcher option set %s is missing.', 'weaver-options-merge' ), $suboption );
							unset( $data );
						}
					}
					else {
			...
			if ( !isset( $data ) && !$msg ) {
				...
				else $msg = __( 'Requested file is missing.', 'weaver-options-merge' );
			}
		}
		if ( !$msg && 'file' == $type ) {
			...
			if ($output_type) {
				...
				if ( is_array( $options_arr ) ) {
					...
					$msg = sprintf( __( 'The unserialize failed on the file %s.', 'weaver-options-merge' ), $outname );
					unset( $data, $_SESSION['womshow'.$theme_abbrev.$postedname] );
				}
			}
			else {
				$msg = sprintf( _x( 'The file %1$s is not a %2$s settings file.', '1 is filename, 2 is theme name', 'weaver-options-merge' ), $outname, ucwords( $theme_name ) );
				unset( $data, $_SESSION['womshow'.$theme_abbrev.$postedname] );
			}
		}
	...
	else $msg = __( 'Choose something to merge.', 'weaver-options-merge' );
...
?>
<div class="wrap">
	<div id="icon-themes" class="icon32"><br /></div><h2><?php _e( 'Weaver Options Merge', 'weaver-options-merge' ); ?> - <?php _e( 'Version', 'weaver-options-merge' );?>  <?php echo WOM_MERGE_VERSION; ?></h2>
	<h3><?php _e( 'Merge two settings files from Weaver II, Aspen, Weaver Xtreme, or Weaver', 'weaver-options-merge' ); ?></h3>
	<p><?php _e( 'This plugin will take two sets of options, and merge them as you specify, and output a settings file.', 'weaver-options-merge' ); ?>
	<?php _e( 'It reads from, but does not write to the database, and it does not matter if the theme is active.', 'weaver-options-merge' ); ?></p>
	<hr />
	<div id="womaccordion">
		<h3>Weaver II</h3>
		<div id="tab1">
	...
	<hr />
	<p><a href="http://joyreynolds.com/"><?php _e( 'Check for more cool stuff from Joy.', 'weaver-options-merge' ); ?></a>
